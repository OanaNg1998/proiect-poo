...
catch(int excep) {if(excep==0) std::cout<<"Obiectul din clasa Matrix ridicat la puterea pow depaseste limita maxima impusa de tipul de date double"<<std::endl;}


try{ std::cout<<" I Matrice *= a II-a Matrice"<<std::endl;
     M3*=M4;
     std::cout<<"Matricea devine :"<<std::endl<<M3;}
catch(int deviation) {if(deviation==1) std::cout<<"Cei doi operanzi de tip Matrix nu respecta regulile operatorului *="<<std::endl;}
...
catch(int error) {if(error==2) std::cout<<"Scalarul cu care este impartita matricea este nul"<<std::endl;}
